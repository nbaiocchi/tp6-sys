...
int main()
{
    ...
    // different API. the calls to `unlink` ensure that we create a
    // fresh semaphore each time.
    sem_unlink("S1");
    S1=sem_open("S1",O_CREAT,0644,  S1_INITIAL_VALUE);
    assert(S1 != SEM_FAILED);
    sem_unlink("S2");
    S2=sem_open("S2",O_CREAT,0644,  S2_INITIAL_VALUE);
